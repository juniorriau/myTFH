...
CREATE DEFINER='myTFHAdmin'@'localhost' PROCEDURE Configuration_def_add(IN `title` VARCHAR(128), IN `templates` VARCHAR(255), IN `cache` VARCHAR(255), IN `flogin` INT(10), IN `private` INT(1), IN `email` VARCHAR(64), IN `timeout` INT(10), IN `pvkey` LONGTEXT, IN `pkey` LONGTEXT, IN `skey` LONGTEXT, IN `countryName` VARCHAR(64), IN `stateOrProvinceName` VARCHAR(64), IN `localityName` VARCHAR(64), IN `organizationName` VARCHAR(64), IN `organizationalUnitName` VARCHAR(64), IN `commonName` VARCHAR(64))
 DETERMINISTIC
...
BEGIN
 ...
 ELSE
  UPDATE `configuration` SET `title`=title, `templates`=templates, `cache`=cache, `flogin`=flogin, `private`=private, `email`=email, `timeout`=timeout, `privateKey`=HEX(AES_ENCRYPT(pvkey, SHA1(skey))), `publicKey`=HEX(AES_ENCRYPT(pkey, SHA1(skey))), `sKey`=HEX(AES_ENCRYPT(skey, SHA1(skey))), `countryName`=countryName, `stateOrProvinceName`=stateOrProvinceName, `localityName`=localityName, `organizationName`=organizationName, `organizationalUnitName`=organizationalUnitName, `commonName`=commonName;
 END IF;
END//

DROP PROCEDURE IF EXISTS Configuration_def_get//
CREATE DEFINER='myTFHAdmin'@'localhost' PROCEDURE Configuration_def_get(IN `challenge` LONGTEXT)
 DETERMINISTIC
 SQL SECURITY INVOKER
...
CREATE DEFINER='myTFHAdmin'@'localhost' PROCEDURE Configuration_def_keys(IN `challenge` LONGTEXT)
 DETERMINISTIC
 SQL SECURITY INVOKER
...
CREATE DEFINER='myTFHAdmin'@'localhost' PROCEDURE Configuration_def_get_dn()
 DETERMINISTIC
 SQL SECURITY INVOKER
