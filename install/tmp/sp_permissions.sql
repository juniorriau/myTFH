...
CREATE DEFINER='myTFHAdmin'@'localhost' PROCEDURE Perms_AddUpdate(IN `name` VARCHAR(128), IN `owner` VARCHAR(128), IN `grp` VARCHAR(128), IN `gr` INT(1), IN `gw` INT(1), IN `usr` VARCHAR(128), IN `ur` INT(1), IN `uw` INT(1), IN `sKey` LONGTEXT)
 DETERMINISTIC
...
CREATE DEFINER='myTFHAdmin'@'localhost' PROCEDURE Perms_DelAll(IN `name` VARCHAR(128))
 DETERMINISTIC
 SQL SECURITY INVOKER
...
CREATE DEFINER='myTFHAdmin'@'localhost' PROCEDURE Perms_DelGroup(IN `name` VARCHAR(128), IN `grp` VARCHAR(128), IN `sKey` LONGTEXT)
 DETERMINISTIC
 SQL SECURITY INVOKER
...
CREATE DEFINER='myTFHAdmin'@'localhost' PROCEDURE Perms_DelUser(IN `name` VARCHAR(128), IN `usr` VARCHAR(128), IN `sKey` LONGTEXT)
 DETERMINISTIC
 SQL SECURITY INVOKER
...
CREATE DEFINER='myTFHAdmin'@'localhost' PROCEDURE Perms_SearchUser(IN `name` VARCHAR(128), IN `usr` VARCHAR(128), IN `sKey` LONGTEXT)
 DETERMINISTIC
 SQL SECURITY INVOKER
...
CREATE DEFINER='myTFHAdmin'@'localhost' PROCEDURE Perms_SearchGroup(IN `name` VARCHAR(128), IN `grp` VARCHAR(128), IN `sKey` LONGTEXT)
 DETERMINISTIC
 SQL SECURITY INVOKER
