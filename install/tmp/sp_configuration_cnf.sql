...
CREATE DEFINER='myTFHAdmin'@'localhost' PROCEDURE Configuration_cnf_add(IN `config` VARCHAR(64), IN `encrypt_key` INT(1), IN `private_key_type` VARCHAR(64), IN `digest_algorithm` VARCHAR(64), IN `private_key_bits` INT(4), IN `x509_extensions` VARCHAR(32), OUT `x` INT)
 DETERMINISTIC
...
CREATE DEFINER='myTFHAdmin'@'localhost' PROCEDURE Configuration_cnf_get()
 DETERMINISTIC
 SQL SECURITY INVOKER
