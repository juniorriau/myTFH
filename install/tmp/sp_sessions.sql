...
CREATE DEFINER='myTFHAdmin'@'localhost' PROCEDURE Session_Search(IN `sessionID` VARCHAR(128), IN `sKey` LONGTEXT)
 DETERMINISTIC
...
CREATE DEFINER='myTFHAdmin'@'localhost' PROCEDURE Session_Add(IN `sessionID` VARCHAR(64), `sessionData` LONGTEXT, `sessionExpire` INT(10), `sessionAgent` VARCHAR(64), `sessionIP` VARCHAR(64), `sessionReferer` VARCHAR(64), IN `sKey` LONGTEXT)
 DETERMINISTIC
 SQL SECURITY INVOKER
...
CREATE DEFINER='myTFHAdmin'@'localhost' PROCEDURE Session_Destroy(IN `sessionID` VARCHAR(64))
 DETERMINISTIC
 SQL SECURITY INVOKER
...
CREATE DEFINER='myTFHAdmin'@'localhost' PROCEDURE Session_Timeout(IN `session_expire` INT(10))
 DETERMINISTIC
 SQL SECURITY INVOKER
