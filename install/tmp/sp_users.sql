...
CREATE DEFINER='myTFHAdmin'@'localhost' PROCEDURE Users_AddUpdate(IN `email` VARCHAR(128), IN `password` LONGTEXT, IN `lvl` VARCHAR(40), IN `grp` VARCHAR(128), IN `sKey` LONGTEXT)
 DETERMINISTIC
...
CREATE DEFINER='myTFHAdmin'@'localhost' PROCEDURE Users_verify(IN `email` VARCHAR(128), IN `password` LONGTEXT, IN `sKey` LONGTEXT)
 DETERMINISTIC
 SQL SECURITY INVOKER
...
CREATE DEFINER='myTFHAdmin'@'localhost' PROCEDURE Users_AddUpdateToken(IN `emailAddy` VARCHAR(128), IN `token` LONGTEXT, IN `sKey` LONGTEXT)
 DETERMINISTIC
 SQL SECURITY INVOKER
...
CREATE DEFINER='myTFHAdmin'@'localhost' PROCEDURE Users_GetToken(IN `emailAddy` VARCHAR(128), IN `sKey` LONGTEXT)
 DETERMINISTIC
 SQL SECURITY INVOKER
...
CREATE DEFINER='myTFHAdmin'@'localhost' PROCEDURE Users_GetLevelGroup(IN `email` VARCHAR(128), IN `sKey` LONGTEXT)
 DETERMINISTIC
 SQL SECURITY INVOKER
...
CREATE DEFINER='myTFHAdmin'@'localhost' PROCEDURE Groups_GetList()
 DETERMINISTIC
 SQL SECURITY INVOKER
...
CREATE DEFINER='myTFHAdmin'@'localhost' PROCEDURE Levels_GetList()
 DETERMINISTIC
 SQL SECURITY INVOKER
