...
CREATE DEFINER='myTFHAdmin'@'localhost' PROCEDURE Auth_CheckUser(IN `emailAddy` VARCHAR(128), IN `pword` LONGTEXT, IN `challenge` LONGTEXT)
 DETERMINISTIC
...
CREATE DEFINER='myTFHAdmin'@'localhost' PROCEDURE Auth_GetLevelGroup(IN `emailAddy` VARCHAR(128), IN `sKey` LONGTEXT)
 DETERMINISTIC
 SQL SECURITY INVOKER
